...
def predict_image(image, model, I):
    ...
    image = np.hstack(
        (np.zeros((r * I, I2, 3)), image, np.zeros((r * I, I2, 3)))
...
def label_grains(image, image_pred, dbs_max_dist=20.0):
    ...
    grains = image_pred[
        :, :, 1
    ].copy()  # grain prediction from semantic segmentation result
    grains[grains >= 0.5] = 1
    grains[grains < 0.5] = 0
    ...
    if len(label_counts[1:]) > 0:
        ...
        if len(grain_data) > 0:
            ...
            dbscan.fit(
                np.vstack((grain_data["centroid-1"], grain_data["centroid-0"])).T
            )
...
def one_point_prompt(x, y, image, predictor, ax=False):
    ...
    if len(masks) >= 1:
        for ind in range(len(masks)):
            if (
                np.sum(masks[ind]) / (image.shape[0] * image.shape[1]) <= 0.5
            ):  # if mask is very large compared to size of the image
    ...
    if len(new_masks) > 0:
        ...
        temp_labels, n_elems = measure.label(
            masks[ind], return_num=True, connectivity=1
        )
        if (
            n_elems > 1
        ):  # if the mask has more than one element, find the largest one and delete the rest
            mask = masks[ind]
            # Find the object with the largest area
            label_counts = np.bincount(temp_labels.ravel())
            largest_label = np.argmax(label_counts[1:]) + 1
...
def sam_segmentation(
    ...
    if remove_large_objects:
        ...
        classifications = classify_points(
            N_neighbors_before, N_neighbors_after, 10, 0, 60, 25
        )
        if len(np.unique(classifications)) > 1:
            cluster_0_mean = np.nanmean(
                np.array(N_neighbors_after)[np.array(classifications) == 0]
            )
            cluster_1_mean = np.nanmean(
                np.array(N_neighbors_after)[np.array(classifications) == 1]
            )
            if (
                cluster_0_mean > cluster_1_mean
            ):  # the cluster with the larger number of neighbors after pruning the components is the one that we want to keep
                all_grains = np.array(all_grains)[
                    np.array(Nodes)[np.array(classifications) == 0]
                ]
            else:
                all_grains = np.array(all_grains)[
                    np.array(Nodes)[np.array(classifications) == 1]
                ]
    ...
    all_grains = merge_overlapping_polygons(
        all_grains, new_grains, comps, min_area, image_pred
    )
    if len(all_grains) > 0:
...
def collect_polygon_from_mask(
    ...
    if (
        ...
        and np.mean(image_pred[:, :, 0][mask]) < max_bg_fraction
    ):
...
def find_connected_components(all_grains, min_area):
    ...
    for i in range(
        len(all_grains)
    ):  # collect the objects that do not overlap each other
...
def merge_overlapping_polygons(all_grains, new_grains, comps, min_area, image_pred):
    ...
    for j in trange(len(comps)):  # deal with the overlapping objects, one connected component at a time
        ...
        if (
            most_similar_polygon.area > min_area
            and most_similar_polygon not in new_grains
        ):
...
def rasterize_grains(all_grains, image):
    ...
    bounds = (
        ...
        image.shape[1] - 0.5,
        -0.5,
    )  # Left, bottom, right, top of the array (bounding box)
...
def create_labeled_image(all_grains, image):
    ...
    boundaries_rasterized = rasterize_grains(
        boundaries, image
    )  # rasterize grain boundaries
    mask_all = np.zeros(
        image.shape[:-1]
    )  # combine grains and grain boundaries into a single mask
...
def onclick(event, ax, coords, image, predictor):
    ...
    if event.button == 1:  # left mouse button for object
        coords.append((x, y))
        sx, sy, mask = one_point_prompt(
            coords[-1][0], coords[-1][1], image, predictor, ax=ax
        )
        ax.figure.canvas.draw()
    if event.button == 3:  # right mouse button for background
        ax.patches[-1].remove()
        coords.append((x, y))
        sx, sy = two_point_prompt(
            coords[-2][0],
            coords[-2][1],
            coords[-1][0],
            coords[-1][1],
...
def onclick2(event, all_grains, grain_inds, ax, select_only=False):
    ...
    for i in range(len(all_grains)):
        if all_grains[i].contains(point):
            ...
            if not select_only:
                ax.fill(
                    all_grains[i].exterior.xy[0],
                    all_grains[i].exterior.xy[1],
                    color="r",
                    alpha=0.5,
                )
                ax.figure.canvas.draw()
...
def onclick_large_image(event, ax, coords, image, predictor, patch_size=1000):
    ...
    if event.button == 1:  # left mouse button for object
        coords.append((x, y))
        patch, patch_origin = extract_patch(
            image,
            (int(np.round(coords[-1][0])), int(np.round(coords[-1][1]))),
            patch_size,
        )
        predictor.set_image(patch)
        sx, sy, mask = one_point_prompt(
            coords[-1][0] - patch_origin[0],
            coords[-1][1] - patch_origin[1],
            patch,
            predictor,
            ax=False,
        )
    ...
    if event.button == 3:  # right mouse button for background
        ...
        predictor.set_image(patch)
        sx, sy = two_point_prompt(
            coords[-2][0] - patch_origin[0],
            coords[-2][1] - patch_origin[1],
            coords[-1][0] - patch_origin[0],
            coords[-1][1] - patch_origin[1],
            patch,
            predictor,
...
def patchify_training_data(input_dir, patch_dir):
    ...
    for image in tqdm(labels):
        ...
        large_image = large_image[
            :, :, 0, np.newaxis
        ]  # only keep one layer and add a new axis
        # Reshape the tensor to have a batch size of 1
        large_image = tf.reshape(large_image, [1, *large_image.shape])
        # Extract patches from the large image
        patches = tf.image.extract_patches(
...
def create_train_val_test_data(image_dir, mask_dir, augmentation=True):
    ...
    if not augmentation:
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(
            (
                train_images,
                train_masks,
                tf.Variable([True] * len(train_images), dtype=tf.bool),
            )
        )
    ...
    test_dataset = (
        test_dataset.shuffle(shuffle_buffer_size)
        .batch(batch_size)
        .prefetch(tf.data.AUTOTUNE)
    )
...
def create_and_train_model(
    train_dataset, val_dataset, test_dataset, model_file=None, epochs=100
):
    """
    ...
    if model_file:
        model = load_model(
            model_file, custom_objects={"weighted_crossentropy": weighted_crossentropy}
        )
    else:
        model = Unet()
        model.compile(
...
def plot_histogram_of_axis_lengths(
    major_axis_length, minor_axis_length, area=[], binsize=0.1, xlimits=None
):
    """
